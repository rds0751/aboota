...
from binary.models import Shopping, BinaryTree
...
class PaymentDetailsView(OrderPlacementMixin, generic.TemplateView):
    ...
    def submit(self, user, basket, shipping_address, shipping_method,  # noqa (too complex (10))
               shipping_charge, billing_address, order_total_without_credit, order_total_with_credit,
        ...
        try:
            self.handle_payment(order_number, order_total_without_credit, **payment_kwargs)
            ...
            # credentials are wrong - that kind of thing.
        ...
        #Now Begins Fun!
        userid = user
        sclbalance = userid.cash_back + float(userid.secondary_cashback)
        usable = float(order_total_without_credit.excl_tax - order_total_with_credit.excl_tax)
        if sclbalance > usable:
            using = usable
        else:
            using = sclbalance
        scl_wallet = WalletHistories()
        scl_wallet.user_id = user.username
        scl_wallet.amount = using
        scl_wallet.type = "debit"
        scl_wallet.comment = "Used Shopping Credit Limit"
        scl_wallet.save()
        payu_wallet = WalletHistories()
        payu_wallet.user_id = user.username
        payu_wallet.amount = float(order_total_without_credit.excl_tax) - using
        payu_wallet.type = "debit"
        payu_wallet.comment = "Paid at the Gateway"
        payu_wallet.save()
        if sclbalance >= usable:
        # if True:
            user_id = user
            
            if user_id.cash_back <= usable:
                usable = usable - user_id.cash_back
                user_id.cash_back = 0
                if user_id.secondary_cashback <= usable:
                    usable = usable - user_id.secondary_cashback
                    user_id.secondary_cashback = 0
                else:
                    user_id.secondary_cashback = user_id.secondary_cashback - usable
                    usable = 0
            else:
                user_id.cash_back = user_id.cash_back - usable
                usable = 0

            user_id.save()

            basket = basket
            total_cashback = float(basket.total_cashback)
            print('------------total-----------', total_cashback)
            user.new_funds += total_cashback*1/4.6
            cashback_wallet = WalletHistories()
            cashback_wallet.user_id = user.username
            cashback_wallet.amount = total_cashback*1/4.6
            cashback_wallet.type = "credit"
            cashback_wallet.comment = "Shopping Cashback"
            cashback_wallet.save()
            user.save()

            userid = Shopping.objects.get(user=user)

            def finduplines(user):
                try:
                    user = Shopping.objects.get(user=user)
                    upline = user.direct
            ...
            for upline in uplines:
                ...
                if upline_user != 'blank':
                    upline_amount = levels['level{}'.format(level)]*amount/4.6
                    upline_user.cash_back += upline_amount*0.9
                    upline_user.save()
                level = level + 1

        else:
            b = balance
            msg = 'Not enough available balance in the wallet'
        ...
        try:
            return self.handle_order_placement(
                ...
                shipping_charge, billing_address, order_total_without_credit, order_total_with_credit, **order_kwargs)

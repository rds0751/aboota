...
WithCreditOrderTotalCalculator = get_class(
    'checkout.calculators', 'WithCreditOrderTotalCalculator')
WithoutCreditOrderTotalCalculator = get_class(
    'checkout.calculators', 'WithoutCreditOrderTotalCalculator')
...
class CheckoutSessionMixin(object):
    ...
    def skip_unless_payment_is_required(self, request):
        ...
        total = self.get_order_totals_without_credit(request.basket, shipping_charge)
    ...
    def build_submission(self, **kwargs):
        ...
        if not shipping_method:
            total_with_credit = total_without_credit = shipping_charge = None
        else:
            shipping_charge = shipping_method.calculate(basket)
            total_with_credit = self.get_order_totals_with_credit(
                basket, shipping_charge=shipping_charge, **kwargs)
            total_without_credit = self.get_order_totals_without_credit(
                basket, shipping_charge=shipping_charge, **kwargs)
        submission = {
            ...
            'order_total_with_credit': total_with_credit,
            'order_total_without_credit': total_without_credit,
            'order_kwargs': {},
            'payment_kwargs': {}}
    ...
    def get_order_totals_with_credit(self, basket, shipping_charge, **kwargs):
        """
        Returns the total for the order with and without tax
        """
        return WithCreditOrderTotalCalculator(self.request).calculate(
            basket, shipping_charge, **kwargs)

    def get_order_totals_without_credit(self, basket, shipping_charge, **kwargs):
        """
        Returns the total for the order with and without tax
        """
        return WithoutCreditOrderTotalCalculator(self.request).calculate(
            basket, shipping_charge, **kwargs)